...
**A two-agent plan → review → implement → review orchestrator for [Claude Code](https://docs.anthropic.com/en/docs/claude-code) (`claude`) and the [Codex CLI](https://github.com/openai/codex) (`codex`).**

It takes two to tango: one agent *leads* (writes the plan and the code) while
the other *follows* (reviews it), and the roles swap freely — per phase, or
even between the plan and implement steps of the same phase. The reviewer is
sandboxed read-only and forced to answer with a machine-parseable verdict, so
"approved" actually means something.
...
| `--stream`                 | Print agent output in real time as it arrives.               |
...
Try it with **`--dry-run`** first: it walks the full loop with stub prompts and
fake verdicts so you can see the flow without spending tokens.
